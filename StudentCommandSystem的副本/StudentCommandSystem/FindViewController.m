...
-(void) finish {
    ...
    if (_scoreTextField.text.length == 0) {
        if (_classTextField.text.length == 0) {
            FindMessageViewController* find = [[FindMessageViewController alloc] init];
            find.student = student;
            [self presentViewController:find animated:YES completion:nil];
        } else {
            if ([_classTextField.text isEqualToString:student.classRoom]) {
                FindMessageViewController* find = [[FindMessageViewController alloc] init];
                find.student = student;
                [self presentViewController:find animated:YES completion:nil];
            } else {
                UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"没有找到该学生" preferredStyle:UIAlertControllerStyleAlert];
                [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:nil]];
                [self presentViewController:alert animated:YES completion:nil];
            }
        }
    } else {
        if ([_scoreTextField.text isEqualToString:student.score]) {
            if (_classTextField.text.length == 0) {
                FindMessageViewController* find = [[FindMessageViewController alloc] init];
                find.student = student;
                [self presentViewController:find animated:YES completion:nil];
